...
# path & source:
current_path=`pwd`
script_path="/nfs/site/disks/km6_io_22/users/paihobon/simulation/simulation_script/auto_pvt/ver01"

source /$script_path/tb_gen/pvt_loop.sh
source /$script_path/configuration_side/read_cfg.sh
source /$script_path/configuration_side/read_supply.sh
source /$script_path/configuration_side/read_corner.sh
...
echo "corners run with tttt extraction: $typ_ex_cornerlist"
echo "corners run with prcs3 & prcf3 extraction: $cross_ex_cornerlist"
}
fi
...
{
nbjob run --target altera_png_normal --qslot /psg/km/phe/ckt/gen --class 'SLES12&&'$mem'G&&'$cpu'C' finesim -np $cpu $testbench.sp -o $testbench >> $current_path/job_log.txt
}
elif [ "$simulator" == "primesim" ]; then 
{
nbjob run --target altera_png_normal --qslot /psg/km/phe/ckt/gen --class 'SLES12&&'$mem'G&&'$cpu'C' primesim -np $cpu -spice $testbench.sp -o $testbench >> $current_path/job_log.txt
}
#to add spectre
else
{
...
echo "#run_ex_corner not specifid, please specified: tttt/prcf3/prcs3"
else
# execute pvt loop for polo
echo "Polo sim with extraction: $run_ex_corner"
echo "NB job submit log" > $current_path/job_log.txt
run_pvt_loop_polo
fi
}
...
if [ "$simulator" == "finesim" ]; then
{
echo "nbjob run --target altera_png_normal --qslot /psg/km/phe/ckt/gen --class 'SLES12&&'$mem'G&&'$cpu'C' finesim -np $cpu $testbench.sp -o $testbench" >> $current_path/rerun.sh
}
elif [ "$simulator" == "primesim" ]; then
{
echo "nbjob run --target altera_png_normal --qslot /psg/km/phe/ckt/gen --class 'SLES12&&'$mem'G&&'$cpu'C' primesim -np $cpu -spice $testbench.sp -o $testbench" >> $current_path/rerun.sh
}
#to add spectre
else
{
